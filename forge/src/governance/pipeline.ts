...
import { seededAdvisories } from '../policy/advisories/seeded.js';
...
export async function validateSpecPipeline(specPath: string): Promise<ValidationResult> {
  const errors: string[] = [];
  ...
  const advisories = seededAdvisories();
  const pe = evaluatePolicy({ spec, advisories });
